...
describe("project API", () => {
  ...
  it("renames a project without changing its other fields", async () => {
    const created = await createProject();
    ...
    await request(app.getHttpServer())
      ...
      .expect(({ body }: { body: ProjectResponse }) => {
        ...
        expect(Date.parse(body.updatedAt)).toBeGreaterThanOrEqual(Date.parse(created.updatedAt));
      });
  });
  ...
  ])("returns the common 400 envelope when renaming with %s", async (_description, name) => {
    const created = await createProject();
